...
bsMat <- bSpline(xs, knots = knots, degree = 3, intercept = T) # using degree = 3 for cubic splines
msMat <- mSpline(xs, knots = knots, degree = 3, intercept = T) # using degree = 3 for cubic splines
...
us1 <- get_u_cond(thetas, ns, qs, np, k, rho, sig)
mean(us1)
(log(1-mean(us1) +2*sqrt(var(us1)/length(us1))) - log(1-mean(us1)))/(rho*k[2])
```
...
# list the decisions to be cycled through
zs <- expand.grid(n = seq(0,200,1), q = seq(-10,10,1))
preds <- NULL
for(i in 1:nrow(zs)){
  #print(zs[i,])
  # For each decision, get the utilities
...
```{r}
us3 <- get_u_cond(thetas, zs3[,1], zs3[,2], np, k, rho, sig)
mean(us1); mean(us3); 
(log(1-mean(us1)) - log(1-mean(us3)))/(rho*k[2])

plot(xs[1:1000], zs3[1:1000,1])
#df3 <- rbind(df2, data.frame(x=xs[1:10^3], n=ns3[1:10^3], t=rep("Regression", 10^3)))
```
