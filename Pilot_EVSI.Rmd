...
bsMat3 <- bSpline(x3s, knots = knots3, degree = 3, intercept = T) # using degree = 3 for cubic splines
...
ptm <- proc.time()
opt2 <- suppressWarnings( BBoptim(par=z, fn=get_u_mc, gr=get_grads_mc,
                                  control = list(gtol=1.e-7, ftol=1.e-10, checkGrad=F),
...
```{r}
get_u_cond <- function(thetas, ns, qs, np, k, rho, sig)
{
  # Expected utility of main trial - note we have assumed rho > 0
  ...
  # Get power
  pows <- 1-pnorm(qs - thetas/sqrt(2*sig^2/ns))
...
```{r}
# Set up basis functions
knots <- seq(quantile(xs, c(0.01, 0.99))[1], quantile(xs, c(0.01, 0.99))[2], l = 12)
bsMat <- bSpline(xs, knots = knots, degree = 3, intercept = T) # using degree = 3 for cubic splines
...
get_u_mc <- function(z, thetas, bsMat, xs, np, k, rho, sig)
{
  ...
  # Get sample sizes from decision rule
  ns <- bsMat %*% z[1:y]
  #ns <- ifelse(ns > 0, ns, 0.0001)
  # Get critical values from decision rule
  qs <- bsMat %*% z[(y+1):(2*y)]
  
  # Penalty for negative sample size
  pen <- -sum(ns[ns < 1])*100000
  
  # Using the thetas associated with the x_1s and thus n_2s, average the conditional 
  # expected utilities
  #u <- pen
  #if(!(pen > 0))u <- -mean(get_u_cond(thetas, ns, qs, np, k, rho, sig))
  #u
  
  u <- -mean(get_u_cond(thetas, ns, qs, np, k, rho, sig))
  u + pen
}
```
...
get_grads_mc <- function(z, thetas, bsMat, xs, np, k, rho, sig)
{
  ...
  f <- 1-pnorm(qs - thetas/sqrt(2*sig^2/ns))
  fd <- -dnorm(qs - thetas/sqrt(2*sig^2/ns))*(-thetas/(2*sqrt(2*ns*sig^2)))
  g1 <- 1 - exp(-rho*(k[1]*thetas + k[2]*(np + ns)))
  g1d <- rho*k[2]*exp(-rho*(k[1]*thetas + k[2]*(np + ns)))
  g2 <- 1 - exp(-rho*(k[2]*(np + ns) + k[3]))
  g2d <- rho*k[2]*exp(-rho*(k[2]*(np + ns) + k[3]))
  ...
  fdq <- -dnorm(qs - thetas/sqrt(2*sig^2/ns))
  
  dfdq <- fdq*(g1 - g2)
  
  # Multiply by derivative with respect to z and sum
  c(-(t(dfdn) %*% bsMat)/length(ns), -(t(dfdq) %*% bsMat)/length(ns))
}

# Check against numerical gradient
z <- rep(1,2*ncol(bsMat))
get_grads_mc(z, thetas, bsMat, xs, np, k, rho, sig)
grad(get_u_mc, z, thetas=thetas, bsMat=bsMat, np=np, k=k, rho=rho, sig=sig)
```
...
ptm <- proc.time()
opt2 <- suppressWarnings( BBoptim(z, get_u_mc, get_grads_mc,
                                  control = list(gtol=1.e-8, ftol=1.e-10, checkGrad=F),
                                  #lower=c(rep(-10^3, 14), rep(-10^3, 14)), 
                                  #upper=c(rep(10^3, 14), rep(10^3, 14)),
                 thetas=thetas, bsMat=bsMat, np=np, k=k, rho=rho, sig=sig) )
z <- opt2$par
proc.time() - ptm
...
df <- data.frame(n=ns, q=qs, x=xs)
df$a <- 1-pnorm(df$q)

ggplot(df[1:10^3,], aes(x, n, colour=a)) + geom_point()
ggplot(df[1:10^3,], aes(x, q, colour=n)) + geom_point()
```
...
for(m in pms[1:10000]){
  suppressWarnings( opt <- optim(c(50,1.95), exp_u, np=np, k=k, rho=rho, m=m, s=sqrt(var_1), sig=sig, #method="L-BFGS-B",
             lower=c(0.001, -5), upper=c(600, 5)) )
  ns2 <- c(ns2, opt$par[1]); ds2 <- c(ds2, opt$par[2]); us2 <- c(us2, -opt$value)
}
proc.time() - ptm
```
...
```{r}
us <- get_u_cond(thetas, ns, qs, np, k, rho, sig); us2 <- get_u_cond(thetas, ns2, ds2, np, k, rho, sig)
mean(us); mean(us2); 
(log(1-mean(us)) - log(1-mean(us2)))/(rho*k[2])
...
# list the decisions to be cycled through
zs <- expand.grid(n = seq(0,100,5), q = c(-10,seq(-2,2,0.5), 10))
preds <- NULL
for(i in 1:nrow(zs)){
  print(zs[i,])
  # For each decision, get the utilities
  us <- get_u_cond(thetas, ns=zs[i,1], qs=zs[i,2], np, k, rho, sig)
  df <- data.frame(x=xs, u=us)
...
```{r}
us1 <- get_u_cond(thetas, ns, qs, np, k, rho, sig); us3 <- get_u_cond(thetas, zs3[,1], zs3[,2], np, k, rho, sig)
mean(us1); mean(us3); 
(log(1-mean(us1)) - log(1-mean(us3)))/(rho*k[2])
