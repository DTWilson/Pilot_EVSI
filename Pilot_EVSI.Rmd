...
get_u_cond <- function(thetas, ns, np, k, rho, sig)
{
  ...
  ds <- qnorm(1-0.025)*sqrt(2*sig^2/ns)
  # Get resulting power

  pows <- 1-pnorm(ds, thetas, sqrt(2*sig^2/(ns)))
...
opt2 <- suppressWarnings( BBoptim(z, get_u_mc,
                 ...
                 ms=ms[1:(10^6/2^i)], bsMat=bsMat[1:(10^6/2^i),], np=np, k=k, rho=rho, sig=sig) )
z <- opt2$par
#proc.time() - ptm
...
get_u_cond <- function(thetas, ns, np, k, rho)
{
  ...
  c <- qnorm(1-0.025)
  # Get resulting power
  pows <- 1 - pnorm(c, ms/sqrt(2*sig^2/ns))
...
print(i)
#ptm <- proc.time()
opt2 <- suppressWarnings( nloptr(z, get_u_mc,
                 opt = list("algorithm"="NLOPT_LN_SBPLX",
                            "ftol_rel"=1.0e-4,
                            "maxeval"=100000),
                 thetas=thetas[1:(10^6/2^i),], bsMat=bsMat[1:(10^6/2^i),], np=np, k=k, rho=rho) )
...
df <- data.frame(s = x1s[1:10^4], m = x2s[1:10^4], n = (bsMat %*% z)[1:10^4])
ggplot(df, aes(m, s, colour=n)) + geom_point() + scale_colour_gradientn(colours = terrain.colors(10)) +
  geom_point(data=expand.grid(s=knots1, m=knots2, n=0), colour="black")
