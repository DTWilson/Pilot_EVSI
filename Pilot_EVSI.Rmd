...
get_u_cond <- function(thetas, ns, np, k, rho, sig)
{
  ...
  # Get power
  pows <- 1-pnorm(qnorm(1-0.025) - sqrt(ns)*thetas/sqrt(2*sig^2))
  pows <- pows*(ns > 0)
...
knots1 <- quantile(x1s, seq(0.01,0.99,l=8))
...
knots2 <- quantile(x2s, seq(0.01,0.99,l=8))
...
# Check against numerical gradient
z <- rep(10,ncol(bsMat))
plot(get_grads_mc(z, thetas, bsMat, np, k, rho),
grad(get_u_mc, z, thetas=thetas, bsMat=bsMat, np=np, k=k, rho=rho) )
```
